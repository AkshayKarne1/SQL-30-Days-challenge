...
with CTE1 as
(
select e.id, e.name as emp_name, e.salary, e.departmentId, d.name as department_name,
rank() over(partition by e.departmentID order by e.salary desc) as rn
from 

employee as e join department as d on 
e.departmentId = d.id)

select id, emp_name, salary, departmentId, department_name, rn from CTE1 where rn<4
